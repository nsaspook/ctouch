 ...
 * A jumper between PORTJ pin6 and +vcc enables debug mode. REMOVE when done.
...
char			lcdstr[18]="CRTFIX E0.93FB |",lcdstatus[18]="D",lcdstatus_touched[18]="Touched",
...
{
int	    code_count;
	for (code_count=0; code_count<=11; code_count++) {
		elocmdout(&elocodes[code_count]);
	};	
...
/*	send data to lcd status panel */
		if (LCD_GO) {							// need to combine this into one string output
